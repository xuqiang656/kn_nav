#!/bin/bash
# Launches the CUDA-free PCT planner node + RViz2 together.
# Usage: ./without_cuda_launch_ros2.sh --skip-tomo --pcd <map.pcd> --tomo <map.pickle>
...
python3 "$ROOT/run_ros2_interactive_without_cuda.py" $ARGS &
NODE_PID=$!

# Wait for the node to be ready (it logs "Interactive ROS2 mode" when done)
echo "Waiting for CUDA-free planner node to initialize..."
