...
Visualizer::Visualizer(const rclcpp::Node::SharedPtr& node, const ParamsConstPtr &params) : node_(node), params_(params) {
  collision_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("~/collision_boxes", 1);
  path_collision_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("~/path_collisions", rclcpp::QoS(1).transient_local());
...
rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr Visualizer::getGraphPublisher(const std::string& ns_prefix) {
  if (graph_pub_.find(ns_prefix) == graph_pub_.end()) {
    graph_pub_[ns_prefix] = node_->create_publisher<visualization_msgs::msg::MarkerArray>("~/" + ns_prefix + "graph", rclcpp::QoS(1).transient_local());
    // Remove old markers.
...
inline void setMarkerPoseFromState(const ob::State* state, visualization_msgs::msg::Marker& marker) {
...
inline void setMarkerPointFromState(const ob::State* state, geometry_msgs::msg::Point& point) {
...
void Visualizer::visualizePlannerGraph(const ompl::base::PlannerData& dat,
  ...
  if (graph_pub->get_subscription_count() == 0) return;
...
void Visualizer::visualizeCollisionBoxesThread(const visualization_msgs::msg::MarkerArray& array) {
  rclcpp::Rate rate(10);
  while (rclcpp::ok()) {
    if (collision_pub_->get_subscription_count()) {
...
geometry_msgs::msg::Pose applyOffsetTo(const geometry_msgs::msg::Pose& offset, const geometry_msgs::msg::Pose& pose) {
...
void Visualizer::visualizePathCollisions(const nav_msgs::msg::Path& path) {
  if (path_collision_pub_->get_subscription_count()) {
    visualization_msgs::msg::MarkerArray array;
    visualization_msgs::msg::Marker marker;
