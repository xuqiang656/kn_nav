...
bool TraversabilityEstimation::readParameters() {
  ...
  nodeHandle_->declare_parameter("grid_map_to_initialize_traversability_map.max_update_rate", 2.0);
  gridMapToInitTraversabilityMapMaxUpdateRate_ =
      nodeHandle_->get_parameter("grid_map_to_initialize_traversability_map.max_update_rate").as_double();
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "TraversabilityEstimation: gridMapToInitTraversabilityMapMaxUpdateRate: %f", gridMapToInitTraversabilityMapMaxUpdateRate_);
  return true;
}
...
bool TraversabilityEstimation::updateServiceCallback(const std::shared_ptr<grid_map_msgs::srv::GetGridMapInfo::Request> request,
  ...
  if (!traversabilityMap_.traversabilityMapInitialized()) {
    RCLCPP_WARN(
      ...
      "Traversability Estimation: update request accepted but traversability map is not computed yet.");
    return false;
  }
...
bool TraversabilityEstimation::requestElevationMap(grid_map_msgs::msg::GridMap& map) {
  (void)map;
  bool expected = false;
  if (!elevationMapRequestInFlight_.compare_exchange_strong(expected, true)) {
    RCLCPP_DEBUG(nodeHandle_->get_logger(), "Elevation map request already in flight.");
    return true;
  }
  ...
  try {
    ...
    elevationMapRequestInFlight_.store(false);
    return false;
  }
...
void TraversabilityEstimation::handle_response(const rclcpp::Client<grid_map_msgs::srv::GetGridMap>::SharedFuture future) {
  try {
    ...
    if (!traversabilityMap_.setElevationMap(elevationMap)) {
      RCLCPP_ERROR(nodeHandle_->get_logger(), "Traversability Estimation: Cannot set elevation map from service response.");
      elevationMapRequestInFlight_.store(false);
      return;
    }
    if (!traversabilityMap_.computeTraversability()) {
      RCLCPP_ERROR(nodeHandle_->get_logger(), "Traversability Estimation: Cannot compute traversability from service response.");
      elevationMapRequestInFlight_.store(false);
      return;
    }
...
bool TraversabilityEstimation::initializeTraversabilityMapFromGridMap(const grid_map::GridMap& gridMap) {
  ...
  for (const auto& layer : elevationMapLayers_) {
    if (!mapWithCheckedLayers.exists(layer)) {
      ...
      RCLCPP_DEBUG(nodeHandle_->get_logger(), "[TraversabilityEstimation::initializeTraversabilityMapFromGridMap]: Added layer '%s'.", layer.c_str());
    }
  }
...
void TraversabilityEstimation::gridMapToInitTraversabilityMapCallback(const grid_map_msgs::msg::GridMap& message) {
  if (gridMapToInitTraversabilityMapMaxUpdateRate_ > 0.0) {
    rclcpp::Time currentTime(message.header.stamp, nodeHandle_->get_clock()->get_clock_type());
    if (currentTime.nanoseconds() == 0) {
      currentTime = nodeHandle_->get_clock()->now();
    }
    if (lastGridMapToInitTraversabilityMapUpdateTime_.nanoseconds() != 0) {
      const double elapsed = (currentTime - lastGridMapToInitTraversabilityMapUpdateTime_).seconds();
      if (elapsed >= 0.0 && elapsed < (1.0 / gridMapToInitTraversabilityMapMaxUpdateRate_)) {
        return;
      }
    }
    lastGridMapToInitTraversabilityMapUpdateTime_ = currentTime;
  }
